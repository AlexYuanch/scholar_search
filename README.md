...
- 学术视野用一个去重列表呈现长期关注、相近同行、合作线索和研究重合，可直接查看画像、追踪或预选到现有学者对比。默认推荐语会说明具体共同方向、时间或关系证据，不再复用统一算法模板。稳定合作者留在合作关系页，代表作移入学术成果，方向延续和选题特征留在研究脉络。“关注机构”只描述真实机构的主题活动与合作记录，不推断实验室团队，不输出机构质量排名。页面不再展示底部通用计算说明，单条推荐依据和机构数据范围仍保留。
...
| `GET /api/authors/{author_id}/intelligence` | 必须登录 | 返回兼容分析字段、四类确定性推荐、领域发现覆盖状态和关注机构；打开学术视野时按需排队 |
