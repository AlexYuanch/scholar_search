...
- 候选卡明确展示身份置信度、当前/历史机构、ORCID（缺失时明确标为未公开）、合并档案数（包括单一档案）和同名区分依据；主搜索与对比搜索都要求用户确认正确身份后才生成画像。
- 学者简介和依据由结构化画像事实按当前界面语言生成，不再复用后端固定语言文本；机构履历来自 OpenAlex 作者档案，学院、实验室和研究院只从该学者论文署名字符串提取，职称与培养阶段没有直接公开依据时整项省略。
...
`npm test` 使用受控工作流与 in-memory Repository 做快速回归，并覆盖 key 加密、缺 key 错误、接口越权、worker 解密，以及专业身份字段不推断职称/学位。`npm run test:db` 启动标准 PostgreSQL，应用 Alembic 迁移，并验证结构、权限、事务发布、分页、会话、跨用户追踪与凭据隔离、搜索缓存持久性与任务去重、重建 Repository 后的数据持久性，以及旧缓存画像从已保存作者/论文署名元数据补全专业身份。外部 OpenAlex/Crossref 全链路另以手动真实数据验收，数据库测试中的受控工作流输出不冒充外部数据验证。
