...
- 前端采用暖黑/米白双主题、低饱和青绿强调色、手写感中文字体和克制留白；首页插画随明暗主题平滑切换，英文界面使用稳定的系统字体与独立响应式字号。顶部“首页”和管理员“系统概况”均为一级内容导航；登录用户最右侧显示圆形头像菜单，可修改昵称、头像、主题色和密码或退出登录。
...
- 全量论文游标分页；文章详情、查询历史和研究追踪在宽屏端使用按剩余容器宽度计算的自适应双栏，在中小屏改为从导航下方展开的完整可滚动面板，避免侧栏遮住、裁切或横向推出主内容。
...
| `POST /api/profile/jobs` | 必须登录、限速 | 已有画像立即返回；首次画像写入历史并幂等创建持久后台任务 |
| `POST /api/profile/stream` | 必须登录、限速 | 兼容接口；供对比等仍需流式结果的旧流程使用 |
...
| `PATCH /api/account/profile` | 必须登录 | 修改昵称、头像样式和主题色 |
| `POST /api/account/password` | 必须登录 | 校验当前密码后修改密码并轮换会话 |
