...
- 匿名访问首先看到简洁产品首页、新手三步指南和匿名标题的真实学者画像操作案例，不会自动弹出登录框；用户提交搜索时才要求登录或注册，成功后自动继续刚才的搜索。案例截图使用高分辨率静态资源，页面内容随滚动克制显隐，并同时说明公开数据遗漏、保守同名消歧、指标含义和 Agent 解读边界。HttpOnly Cookie 会话保护查询、私有历史和研究追踪。
