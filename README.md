...
- 已有画像立即从 PostgreSQL 返回；超过刷新阈值或用户点击“重新生成画像”时，只向 `refresh_jobs` 幂等排队，由 worker 异步重跑 OpenAlex、ORCID、Crossref、身份裁决和全部 Agent，旧画像在新版本发布前继续可用。
...
- 候选卡明确展示身份依据分组、主要/其他关联机构、ORCID、合并档案数、最近发表和研究主题，支持按机构、主题、ORCID 与学术指标筛选排序；默认“身份依据优先”不表示系统已经确定目标身份。主搜索与对比搜索都要求用户确认正确身份后才生成画像。主要关联机构按 OpenAlex 最近六年内覆盖的不同发表年份数确定，最近年份和全职业覆盖数只用于同分排序。
