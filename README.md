...
- 管理员可从顶部“系统概况”查看近 5 分钟在线用户与匿名访客、带时间刻度的访问/搜索趋势、热门学者、后台任务和近期异常；点击“当前在线”可查看登录用户名、活动时间和匿名会话编号。普通用户无法读取统计接口。现有 `admin` 账号在迁移后成为受保护的超级管理员，可在看板中授予或撤销其他账号的只读管理员权限。
