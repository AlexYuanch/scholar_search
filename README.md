...
- 图谱首次构建走单学者全量 OpenAlex，之后按最近成功时间减 30 天使用免费计划支持的 `from_publication_date` 重叠增量；显式重建才重新读取该学者全量论文。只有用户打开“研究图谱”栏时才按需排队，普通画像访问不会提前消耗共享额度；新鲜图谱的重复刷新直接复用当前版本，完整重建只在上一轮失败后开放。OpenAlex 分页不完整时整批不写，Crossref 失败不覆盖此前成功的出版字段。
