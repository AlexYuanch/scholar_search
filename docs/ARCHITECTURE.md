...
8. 前端学术视野把四类推荐按 OpenAlex 作者 ID 合并为一个列表，复用现有画像、追踪和学者对比入口；“全部”视图选择该候选确定性指数最高的实际角色，不再固定优先某一类别。推荐语引用共同方向、候选代表成果及该候选最突出的时间、扩散、关系或问题—方法证据，避免只替换身份文本的模板重复；详细指数仍只保留在折叠依据中。页面不再渲染自我分析、代表作、团队卡、第二套比较组件或底部通用计算说明。确定性代表作改由学术成果栏展示，方向延续和选题特征继续由研究脉络承载。
