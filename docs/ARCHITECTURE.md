...
匿名前端先展示 Hero 搜索、原创科研插画、三项核心能力和数据可信说明，不自动打开登录框。`LandingGuide` 在其后提供三步使用指南、真实李飞飞候选/工作流/概览/合作网络截图和能力边界；截图作为压缩静态资源随构建发布，不包含账号、密钥或私有数据。`LandingHero` 只编排现有搜索状态，不新增匿名 API：匿名搜索把姓名保存在 `App` 组件状态并打开可关闭的认证弹窗；注册或登录成功后直接执行该姓名，关闭则取消本次动作但保留输入。后端查询路由没有开放匿名权限。
