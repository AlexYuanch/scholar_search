...
7. `GET /api/authors/{author_id}/intelligence` 保持旧 dimensions、representative works、recommendations 与 teams 字段，同时增加 `discovery`、`institutions`、`peer_candidates` 和候选分页元数据；读取接口不再同步推进领域发现，只负责读取并按需要排队。`GET /api/authors/{author_id}/intelligence/peers?cursor=&limit=20` 按“图谱就绪优先、发现排名次之”返回下一批已发现候选，默认和最大数量均为 20；已就绪候选带确定性推荐，未就绪候选只带基础身份、方向和发现规模。`POST /api/authors/{author_id}/intelligence/discover` 继续幂等请求发现任务。旧 scholar/team 比较模式与反馈接口保持兼容。
